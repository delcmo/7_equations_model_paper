...
\end{subequations}
%
\tcr{the $\mathbb{aaa}$ symbol is reserved for rank-2 tensors. Please change to scalar, vector (math mbold), and tensor (mathbb) as appropriate to avoid confusion\\} \tcb{done} \\ \tcr{you still had Gamma in bold. I changed the definition of the Gammakj command\\ Finally, the M exchange term should be a vector (bold) I believe\\} \tcb{done}
...
%
\tcr{in the definition of f, A should not be there in front of l because A is already in l} \tcb{you are right}
...
\end{align}
\tcr{this is not complete for g. I think it is missing u cross f on the RHS, or should it be f cross u? Please check} \tcb{you are right. It is f cross u}
\begin{align}
...
\end{multline}
%
\tcr{later I see $\alpha^*$ but it is not defined here. There is no $\alpha_\infty$\\} \tcb{done}
...
\end{align}
%
\tcr{alpha is missing in the first term on the RHS of the continuity eq. below. Please, review all equations carefully}\tcb{ok}
...
\end{multline}
\tcr{also, how about not writing $\grad^* \cdot \left( a \grad^* b \right)^*$, the $^*$ is doubled for the inner gradient, but replacing as $\grad^*\cdot \left( a^* \grad^* b^*\right)$} \tcb{What about $\grad^*\cdot \left( a \grad b\right)^*$}
%
...
\end{multline}
\tcr{order of $\otimes$ in the above?}\tcb{I checked it}
%
...
%
\tcr{I cannot go from 35d to 37. The $\gamma$ and the $\gamma-1$ do not make sense. Something must be wrong} \tcb{I fixed it}
...
\alpha_{k,0} \gamma_k \left( P_{k,0} +alpha is  P_{k\infty} \right) \div \left( \mbold u_k A\right)_0 = \\
