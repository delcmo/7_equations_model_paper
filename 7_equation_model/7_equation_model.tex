...
\begin{align}
  ...
  &= A \mu_P (P_{k} - P_{j}) - \mathbb{S}_{k\to j} \,,
\end{align}
\begin{align}
...
where $\alpha_k$, $\rho_k$, $\mbold u_k$ and $E_k$ denote the void fraction, the density, the velocity vector and the total specific energy of phase $k$, respectively. The void fraction, mass, momentum and energy exchange terms from phase $k$ to phase $j$ are denoted by the symbols $\mathbb{S}_{k \to j}$, $\Gammakj$, $\mathbb{M}_{k \to j}$ and $\mathbb{E}_{k \to j}$, respectively, are assumed to be determined by the method of rational thermodynamic \cite{Truesdell} and thus will only yield positive terms in the right-hand side of the entropy equation. They also obey to the following closure relations:
%
\begin{subequations}\label{eq:exch-terms}
\begin{equation}
\mathbb{S}_{1 \to 2} + \mathbb{S}_{2 \to 1} = 0 \, ,
\end{equation}
%
\begin{equation}
...
%
\begin{equation}
\mathbb{M}_{1 \to 2} + \mathbb{M}_{2 \to 1} = 0 \, ,
\end{equation}
%
\begin{equation}
\mathbb{E}_{1 \to 2} + \mathbb{E}_{2 \to 1} = 0 \, .
\end{equation}
%
\end{subequations}
%
\tcr{indeed, since we give a convention for scalar, vector, tensor here, we should respect it above\\}
...
%
\tcr{check sign. If Q is def negative, then I think we need $-$\\}
...
\end{align}
\tcr{this is not complete for g. I think it is missing +u cross f on the RHS}
\begin{align}
...
\end{align}
\tcr{alpha is missing in the first term on the RHS of the continuity eq. Please, review all equations carefully}
\begin{multline}\label{eq:sev_equ-with-diss-terms-cont_case_one_scaled}
\partial_{t^*} \left( \alpha_k \rho_k A \right)^* + \grad^* \cdot \left( \alpha_k \rho_k \mbold u_k A \right)^* = \\ \frac{1}{\Pe^\kappa_{k,\infty}}\grad^* \cdot \left(A \kappa_k \grad^* \rho_k \right)^* +
...
\frac{1}{\Re_{k,\infty}}\grad^* \cdot \left( A \alpha_k \mu_k \rho_k \grad^s \mbold u_k \right)^* \\ + \frac{1}{\Pe_{k,\infty}^\kappa} \grad^* \cdot\left( A \alpha_k \kappa_k  \mbold u_k \otimes \grad^* \rho_k \right)^* + 
\frac{1}{\Pe_{k,\infty}^\beta} \grad^* \cdot \left( A \beta_k \rho_k \mbold u_k \otimes \grad \alpha_k \right)^*
\end{multline}
...
\frac{1}{\Pe_{k,\infty}^\kappa} \grad^* \cdot \left( A \alpha_k \kappa_k \grad \left( \rho_k e_k \right) \right)^* + \frac{M^2_{k,\infty}}{\Pe_{k,\infty}^\kappa} \grad^* \cdot \left( A\alpha_k \kappa_k \frac{||\mbold u_k||^2}{2} \grad \rho \right)^* + \\
