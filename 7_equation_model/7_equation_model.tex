...
\end{subequations}
%
\tcr{the $\mathbb{aaa}$ symbol is reserved for rank-2 tensors. Please change to scalar, vector (math mbold), and tensor (mathbb) as appropriate to avoid confusion\\} \tcb{done} \\ \tcr{you still had Gamma in bold. I changed the definition of the Gammakj command\\ Finally, the M exchange term should be a vector (bold) I believe\\}
...
%
\begin{equation}
\mathbb{\Gamma}_{1 \to 2} + \mathbb{\Gamma}_{2 \to 1} = 0 \, ,
\end{equation}
%
\begin{equation}
M_{1 \to 2} + M_{2 \to 1} = 0 \, ,
...
\end{subequations}
%
\tcr{indeed, since we give a convention for scalar, vector, tensor here, we should respect it above\\} \tcb{ok}
...
%---------------------------------------------------------------------------------
\subsection{Mathematical properties and entropy equation of the SEM without viscous regularization}\label{eq:sem-ent-wv}
%---------------------------------------------------------------------------------
...
whose eigenvalue (speed) is $\mbold u_{int}$. An entropy relation can be derived \tcr{for that equation alone} (by multiplying it by $\alpha_k$)
...
%
\tcr{in the definition of f, A should not be there in front of l because A is already in l}
...
\end{align}
\tcr{this is not complete for g. I think it is missing u cross f on the RHS, or should it be f cross u? Please check}
\begin{align}
\mathbb{g}_k &= \alpha_k A \mu_k \rho_k \grad^s \mbold u_k + \mbold u_k \otimes \mbold f_k 
\end{align}
\begin{align}
...
We consider the case where the relaxation coefficients are set to zero, that is, \tcr{any justification for doing this?}
...
\end{multline}
%
\tcr{later I see $\alpha^*$ but it is not defined here. There is no $\alpha_\infty$\\}
...
\end{align}
%
\tcr{alpha is missing in the first term on the RHS of the continuity eq. below. Please, review all equations carefully}
...
\end{multline}
\tcr{also, how about not writing $\grad^* \cdot \left( a \grad^* b \right)^*$, the $^*$ is doubled for the inner gradient, but replacing as $\grad^*\cdot \left( a^* \grad^* b^*\right)$}
%
...
+ \frac{1}{\Pe_{k,\infty}^\kappa} \grad^* \cdot\left( A \alpha_k \kappa_k  \mbold u_k \tcr{\otimes} \grad^* \rho_k \right)^* 
+ \frac{1}{\Pe_{k,\infty}^\beta} \grad^* \cdot \left( A \beta_k \rho_k \mbold u_k \tcr{\otimes} \grad \alpha_k \right)^*
\end{multline}
\tcr{order of $\otimes$ in the above?}
%
...
\tcr{+} \frac{M_{k,\infty}^2}{\Pe_{k,\infty}^\beta} \rho_k \frac{|| \mbold u_k^2 ||}{2} \div \left( \beta_k A \grad \alpha_k \right)
\end{multline}
\tcr{shouldn't the $-$ be a $+$? $h = \alpha A k \grad (\rho e) -u^2/2 f$ and $g=A\mu \alpha \rho \grad u + uf$\\
Thus, $h+ug =  \alpha A k \grad (\rho e) \textcolor{green}{+}u^2/2 f + A\mu \alpha \rho \grad u$\\ \vspace{1mm} \\
Furthermore, why is $\rho u^2/2$ outside of the divergence i the very last term?\\}
%
\end{subequations}
%
...
is obtained for $\Re_{k,\infty}$, $\Pe_{k,\infty}^\kappa$, and $\Pe_{k,\infty}^\beta$ in the two limit cases (a) and (b), it will impose a condition onto the definition of the phasic viscosity coefficients $\mu_k$, $\kappa_k$ and $\beta_k$. For brevity, the superscripts $^*$ are omitted in the remainder of this section.  \tcr{to be clearer, should it be "`remainder of section 4"'?}
...
%
\tcr{I cannot go from 35d to 37. The $\gamma$ and the $\gamma-1$ do not make sense. Something must be wrong}
\begin{multline}\label{eq:asympt_equ3_ener}
A \alpha_{k,0} \left[ \partial_t  P_k  + (\gamma_k-1)\mbold u_k \cdot \grad  P_k \right]_0 + \\ (\gamma_k-1) \alpha_{k,0} \div \left[ A \mbold u_k P_k \right]_0 +
