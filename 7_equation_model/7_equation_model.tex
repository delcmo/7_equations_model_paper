...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
In this section, we recall the Seven-Equation two-phase flow model and its main mathematical and physical properties.
%
%---------------------------------------------------------------------------------
...
\tcb{I changed the exchange terms in each of the equations above.} 
%
\begin{subequations}
...
%---------------------------------------------------------------------------------
\subsection{Some mathematical properties and entropy equation of the SEM without viscous regularization}\label{eq:sem-ent-wv}
%---------------------------------------------------------------------------------
...
Next, we investigate the sign of the phasic and total entropy equations \emph{without viscous regularization}; the total entropy equation being obtained by summing over the phasic entropy equations. Since the exchange source terms are derived from the rational thermodynamic method \cite{Truesdell}, it is assumed that they will only yield positive terms in the right-hand side of the total entropy equation and thus, can be omitted. Based on the previous assumption, we now consider the SEM model with only pressure and velocity relaxation terms:
...
\end{equation}
%
From a physical prospective, \eqt{eq:tot-ent-res-sct4} tells us that the total entropy of the system increases as a function of time. From a numerical prospective, the pressure and velocity relaxation terms add dissipation to the system of equations.
...
last term in $\mbold f$ was changed. I think it is correct now.} \tcb{Agreed}
...
%
\tcr{check sign} \tcb{I do not see anything wrong}
...
%
\tcr{is the following remark useful in the paper? Can it be removed completely?} \tcb{done}
%\begin{remark}
...
In this section, we derive the non-dimensionalized SEM to outline possible leads on the definition of the viscosity coefficients to ensure the correct scaling of the dissipative terms for all-Mach flows.
...
% 
\tcr{you cannot have $\grad^* \cdot$, it should be $\nabla^* \cdot$. Please change where nedded.} \tcb{done}
...
\end{subequations}
%
where the phasic numerical Reynolds ($\Re_{k,\infty})$ represents the ratio of fluid inertia force to viscous forces, and P\'eclet ($\Pe_{k,\infty}^\kappa$ and $\Pe_{k,\infty}^\beta$) represents the ratio of advection rate to diffusion rate. These numerical numbers are defined as:
...
ratio of advection rate to diffusion rate} \tcb{done}
%
\begin{equation}
\label{eq:ref_numb}
