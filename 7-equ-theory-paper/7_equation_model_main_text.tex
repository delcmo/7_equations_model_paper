...
\tcr{here we talk about HCSE and HNCSE but we never mention why we talk about this. Should we not introduce this first?}
...
the math around the definition of the path needed? It sounds complicated without necessarily shining light on important information}
...
that is path-dependent. Furthermore, the notion of shock, rarefaction and shocks waves \tcr{not balanced} has also been extended to HNCSE,
...
Another approach \tcr{to do what? I guess: obtain a numerical solution to the SEM} that is of interest for this paper consists in considering the 
vanishing viscosity solution obtained by adding a parabolic viscous regularization to the HNCSE.
...
numerical solution to the entropy weak solution \tcr{both weak and entropy?}.
%Of course if the 
...
\end{subequations}
%
\tcr{what's the problem with mathbb, I cannot see the letter g above. I looked before, it happened too. some conflicts in the latex package?}
