...
\tcr{are you sure these are HCSE that they start with? } \tcb{This is one is done in DEM paper}
...
\tcr{thermal is the definition of the C-E expansion, but later we call the kapila model the mechanical equil model, I am confused as of 
it is mechanical or thermal equil then ...} \tcb{this is the first time I read the term thermal for Kapila. I would rather use Kapila as it is used in other papers. It is mechanical equilibrium}
...
Instead the theory developed by Del Maso-Le Floch-Mariat is used to generalize the notions of weak solutions by introducing a path \tcb{in order to define
the non-conservative product at points of discontinuity.}
\tcr{a path of what, for what? sentence feels incomplete} \cite{dlm} \tcb{also we already talked of path in the introduction}. 
...
\tcr{If you look at the previous version of this paragraph, we had the work of bianchine (2005) complete by le floch in 1989, anachronism,
which itself was later introduce by le floch in 1988, another anachronism. So, I am going to move things around and/or remove these temporal indications
because they do not make sense. Check this carefully. If I changed some meaning, then we need to talk.}
...
\tcr{check that the references go with the theorems. I moved things around a bit}
...
for HNCSE is also valid for HCSE \tcb{under the condition that the nonconservative form may be written under a conservative form}\tcr{like what? talk}: this remark is of importance since it will guarantee, from a 
...
equation for Euler equations (see \tcr{Appendix~B} \tcb{I think Appendix B is more appropriate} in \cite{Marco_paper_low_mach}). $\mathcal{R}_1$ is known to be positive when 
...
Assume the seven-equation model is regularized as shown in \eqt{eq:sev_equ-with-diss-terms} and that the corresponding entropy equation given in \eqt{eq:ent-res-7-eqn-diss-terms3} holds.
The left-hand side of \eqt{eq:ent-res-7-eqn-diss-terms3} is non-negative if and only if the phasic entropy function $s_k$ is concave with respect to $e_k$ and $\rho_k^{-1}$ and, 
\begin{align}
...
the entropy reaches its spatial minimum, one concludes that $\alpha_k \rho_k \frac{D s_k}{Dt} \geq 0$. For the minimum entropy principle to hold for each phase $k$, i.e. $\frac{D s_k}{Dt}\geq 0$, positivity of the product $\alpha_k \rho_k$ is also required. To prove positivity of the product $\alpha_k \rho_k$, we need to derive an expression for the dissipative term $\mbold l_k$ since it is present in the phasic continuity equation \eqt{eq:sev_equ-with-diss-terms-cont}. Note that the inequality $\alpha_k \rho_k \frac{D s_k}{Dt} \geq 0$, holds {\it independently} of the definition of the dissipative term $\mbold l_k$
in the volume fraction equation.
\tcr{I do not understand the linkage between the last 3 sentences. The second to last sentence seems misplaced or the relationship between alpha rho and l
is not yet known but the sentence assumes that the readers knows something about it, which is weird because that relationship is the purpose of
the next section...} \tcb{I modified it. Have a look}
%
%----------------------------------------------------------------------------------------------------------------------------
...
such as $ \partial_d \alpha_k = 0$ and $\partial_{dd} \alpha_k \leq 0$ for $1\le d \le {\tt dim}$. \tcr{check} \tcb{I think this is correct but I remember using the Hessian matrix in multi-D}.
...
\tcr{did you update this. I thought a $q$ term was missing according to a reviewer...?!?} \tcb{done}
...
finite element method. \tcr{This is in contrast with the typical discretizations schemes adopted for 
...
and thus have opted for a less traditional discretization technique for HNCSE; second, several open-source, 
parallel, continuous finite element libraries are available (e.g., MOOSE \cite{moose} which was employed here),
thus possibly enabling the dissimnation of multi-phase flows using such tools.} \tcb{that's good}
...
we do not need to compute the interfacial flux for example}\tcr{maybe you can fit something above, around what I have highlighted in red}

We illustrate the ability of the proposed viscous regularization to stabilize the 
seven-equation two-phase flow equations using two test cases. \tcb{It will be three after I add the third test}\tcr{Status update?}
...
%
\tcr{the SGEOS was used TWICE before and I think this is the only time it is given correctly. Do we even need to give it before?} \tcb{I corrected all of them}
...
\end{align}
%
\tcr{so is $ e_k -q_k = C_{v,k} T_k$ or is a $q_k$ missing in the second eq.?} \tcb{The second equation is correct. }
...
%-----------------------------------------------------------
%
In this example, we set the relaxation coefficients $\mu_P$ and $\lambda_u$ to large values; the pressures and velocities in each phase should become identical, \tcr{as expected by performing a Chapman-Enskog expansion} \tcb{ok}. 
...
for the above cases (1) and (2). \tcr{I was going to say ``not true, the figures below do not show cases 1 and 2'', because I can barely notice the light blue. Please you another color!} \tcb{done}
...
\tcr{this is only theoretical since we do not show numerical values; should we keep on emphasizing this?}, \tcb{Well having positivity of the entropy residual is the heart of the method. I would keep it. We are not even using the entropy residual to compute the visc. coeff. in our numerical results. Also, when we compute the entropy residual, it is not always positive because we do not have a good way to accurately compute it.}
convergence of the numerical solution to an entropy solution \tcr{for concave physical phasic entropy functions $s_k$ {\it is this needed in the conclusions, sounds heavy}} \tcb{we should add it if we want to be precise}, 
