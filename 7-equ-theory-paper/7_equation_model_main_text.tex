...
by integrating the one-phase flow hyperbolic conservative system of equations (HCSE), i.e. Euler equations, weighted by a characteristic or indicator function for each phase \cite{DrewPassman}.
...
The systems of two-phase flow equations are non-conservative system of equations and are usually solved using discontinuous discretization schemes (finite volume and discontinuous 
Galerkin approaches). By assuming that the non-conservative system of equations is hyperbolic, an exact Riemann solver could be used but is often ruled 
...
\tcr{here we talk about HCSE and HNCSE but we never mention why we talk about this. Should we not introduce this first?} \tcb{done}
...
globally smooth solution may not occur because of the nonlinearity of the flux functions. For hyperbolic conservative systems of equations (HCSE), the concept of a weak
solution was introduced by Lax \cite{Lax} to guarantee the existence of a global solution; however, uniqueness of the solution is lost because the problem may
allow infinitely many weak solutions. Thus, an additional condition, the``entropy condition,'' is usually imposed to ensure convergence to a
...
entropy condition, thereby guaranteeing that the numerical computation captures the physically relevant solution. However, for hyperbolic non-conservative systems 
of equations (HNCSE) such as the seven-equation model, the theory of Lax cannot be invoked to establish weak and entropy solutions in the sense of distributions. Instead, the work by Del Maso-Le Floch-Mariat (DLM) \cite{dlm} must be used: the non-conservative products are defined as a bounded Borel 
measure by introducing a Lipschitz family of path.
%$\phi: [0,1] \times \mathbb{R}^n \times \mathbb{R}^n$ such that 
%$\phi(0, U_L, U_R) = U_L$ and $\phi(1, U_L, U_R) = U_R$. 
\tcr{this would be a bit incomprehensible for any reader. what are we trying to say? is it important to mention $U_L$ and $U_R$ here? if
the math around the definition of the path needed? It sounds complicated without necessarily shining light on important information} \tcb{done}
...
are extended to HNCSE in \cite{lefloch_1988,lefloch_1989}, leading to the derivation of a generalized Rankine-Hugoniot (RH) jump condition
that is path-dependent. Furthermore, the notion of rarefaction and shocks waves \tcr{not balanced} \tcb{done} has also been extended to HNCSE,
...
Another approach that is of interest for this paper to establish the existence of a weak solution of non-conservative terms,
\tcr{to do what? I guess: obtain a numerical solution to the SEM} \tcb{done}
...
vanishing viscosity system is unique as the vanishing viscosity tends to zero (Theorem 1, page 229 in \cite{bianchini_bressan_2005}). Moreover, Le 
...
numerical solution to the entropy solution \tcr{both weak and entropy?} \tcb{done}.
%Of course if the 
...
$u_{int}$ are not unique, see \cite{Saleh_phd_thesis} for instance. The results presented in this paper apply to \eqt{eq:liq-7-eqn-sect5},
...
real (as long as the equation of state yields a real-valued sound speed) but not necessarily distinct. However, the seven-equation model admits a set of linearly
...
thus ensuring hyperbolicity of the model.
Hyperbolicity is extremely valuable property for the development of numerical methods since it is required of well-posed hyperbolic systems.
...
of phase $k$, as in the case of the single-phase Euler equations \cite{jlg,Marco_dissertation}, and thus is also a positive term.
...
Instead the theory developed by DLM \cite{dlm} is used to generalize the notions of weak solutions by introducing a path \cite{dlm}. Bianchini et al. 
in \cite{bianchini_bressan_2005} extended the notion of vanishing viscosity solution to HNCSE (see Theorem 1. on page 229). This work was completed by 
...
consistency of the viscous stabilization will be ensured as the SEM devolves to the conservative Euler equations. \tcb{I am not sure the wording "continuity of the 
viscous stabilization" is the most meaningful} \tcb{done}
...
principle, one needs to show that the phasic entropy residual $R_{e,k} := \frac{Ds_k}{Dt}$ is positive, which requires (i) positivity of the phasic product $\alpha_k \rho_k$
and (ii) positivity of the right-handside of \eqt{eq:ent-res-7-eqn-diss-terms_rearranged}.
\sct{sct:positivity} is devoted to proving positivity of the right-handside of \eqt{eq:ent-res-7-eqn-diss-terms_rearranged} by choosing an adequate expression for the viscous
fluxes. we also show that $\alpha_k \rho_k$ remains nonnegative under certain conditions. Then, in \sct{sct:end-min-pr}, the minimum entropy principle is
proved by using results from \sct{sct:positivity}.
%
%----------------------------------------------------------------------------------------------------------------------------
\subsection{A definition of the viscous fluxes based on a entropy condition}
%-----------------------------------------------------------------------------------------------------------------------------
%
\tcr{not sure about the title. This section is not only about  $\alpha_k \rho_k$  ...}\tcb{done}
...
To do so, we need to derive an expression for the dissipative term $\mbold l_k$ since it is present in the phasic continuity equation \eqt{eq:sev_equ-with-diss-terms-cont}.
%
%----------------------------------------------------------------------------------------------------------------------------
...
\end{equation}
%
Let us assume that the 1-D 
\tcr{do we have to use one 1D, can't we say  $ \partial_q \alpha_k = 0$ and $\partial_{qq} \alpha_k \leq 0$, fpr $q=x,y,z$?}
phasic volume fraction is initially bounded within the interval $[0;1]$ and that a maximum is reached at a given point $x_0 \in \mathcal{R}$
...
Similarly, if a minimum is reached in $x_0$ (i.e., $\partial_x \alpha_k = 0$ and $\partial_{xx} \alpha_k \geq 0$),
...
Note that the viscous term in the phasic volume fraction equation is needed as illustrated in the following example. Consider a test case with uniform initial 
phasic pressure, velocity, temperature and an initial step discontinuity in the volume fraction. Under such conditions and using the definitions given in \eqt{eq:int_variables_def}, 
...
i.e., $\mbold l_k := \mbold 0$, the test case with the above initial condition would result in a numerical solution with spurious oscillations in the vicinity of the discontinuity. 
\end{my_remark}
%
...
and the resulting equation is identical to the regularized continuity equation of the Euler equation derived in \cite{jlg} when defining $\hat{\rho}_k = \alpha_k \rho_k$:
%
...
\end{equation}
%
Following Lemma 3.1 of \cite{jlg} and using \eqt{eq:reg-euler-qu}, a nonnegative density principle can be proved for the quantity $\hat{\rho}_k$: this allows us to 
conclude that the quantity $\alpha_k \rho_k$ remains nonnegative under the assumption $\beta_k = \kappa_k$. Note that unlike Lemma 3.2 of \cite{jlg}, the density 
$\hat{\rho}_k$ cannot be proved strictly positive because the phasic volume fraction $\alpha_k$ can be locally equal to zero.
\tcr{but do we care. if alpha=0, we are in the single-phase case, locally at least, and there we can used Guermond-Popov, and density is strictly positive, no?}
%
...
previously illustrated in Subsection 2.3 of \cite{jlg} for Euler equations.
\end{my_remark}
%
...
\tcr{ that sounds weird, if we start with alphak=0.5, your sentence implies we will always remain at alphak=0.5 ....} \tcb{done}, 
...
\end{subequations}
%
\tcr{what's the problem with mathbb, I cannot see the letter g above. I looked before, it happened too. some conflicts in the latex package?} \tcb{done}
...
The phasic entropy residual is proved to be nonnegative in \sct{sct:end-min-pr}. When numerically computed, however, one can observe that the phasic entropy residual $\mathcal{R}_{e,k}$,
can experience negative variation in the vicinity of a shock, as previously noted  in \cite{Guermond_Pasquetti}: this is due to the strong variations of the phasic entropy 
residual in the shock regions. \tcr{should we not add something like we are sampling an numerical approximation to the residual that tends to a Dirac distribtion} We refer the reader to Section 4 of \cite{Guermond_Pasquetti} for further discussions.
\end{my_remark}
\tcb{This is maybe not the best place to have this remark.} \tcr{Maybe in a place where we say that we use the abolute value of the residual rather than just the residual?}
%
%---------------------------------------------------------------------------------
