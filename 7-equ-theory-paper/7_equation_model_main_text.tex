...
\tcr{except for the discrete equation method (DEM) for two-phase flows, \cite{SEM} and the references contained therein, which reverses the order of 
...
a subset of non-conservative systems. \tcr{For the five-equation two-phase model of Kapila et al. \cite{Kapila_2001} the non-conservative nature 
...
and theoretical importance: the multi-phase low-Mach regime and the \tcr{reduced} multiphase \tcr{mechanical} equilibrium \tcr{model} 
\tcr{(treated as a Chapman-Enskog type expansion)}.
...
$u_{int}$ are not unique, see \tcr{\cite{BaerNunziato, GallouetHerardSeguin, Berry_2005}} for instance. 
...
or, after \tcr{using the chain rule to combine partial derivatives},
%
