...
\tcr{are you sure these are HCSE that they start with? } \tcb{This is one is done in DEM paper} \tcr{thus, I have added here the SEM reference. If you agree with the setence, we can delete this series of comments}
...
and theoretical importance: the multi-phase low-Mach regime and the multi-phase \tcr{thermodynamical/mechanical} equilibrium treated as a Chapman-Enskog expansion.
...
it is mechanical or thermal equil then ...} 
\tcb{this is the first time I read the term thermal for Kapila. I would rather use Kapila as it is used in other papers. It is mechanical equilibrium}
\tcr{I didn't say mechanical for kapila. I am talking about CE. I am browsing the web for a definition of what a CE expansion here. Let me add thermodynamical in red and we ask Ray}
...
the entropy reaches its spatial minimum, one concludes that $\alpha_k \rho_k \frac{D s_k}{Dt} \geq 0$. For the minimum entropy principle to hold for each phase $k$, i.e., $\frac{D s_k}{Dt}\geq 0$, positivity of the product $\alpha_k \rho_k$ is also required. To prove positivity of $\alpha_k \rho_k$, we need to derive an expression for the dissipative term $\mbold l_k$ since it is present in the phasic continuity equation \eqt{eq:sev_equ-with-diss-terms-cont}. Note that the inequality $\alpha_k \rho_k \frac{D s_k}{Dt} \geq 0$, holds {\it independently} of the definition of the dissipative term $\mbold l_k$
in the volume fraction equation.
\tcr{Ok. I am less confused but still a bit. Here, we say that deriving $l$ is required for positivity of alpha rho and in the next sentence we say that  $\alpha_k \rho_k \frac{D s_k}{Dt} \geq 0$ is independent of $l$. There is still some ambiguity here}
%
%----------------------------------------------------------------------------------------------------------------------------
...
\tcr{check} \tcb{I think this is correct but I remember using the Hessian matrix in multi-D} \tcr{can you tell me more? Why the Hessian}
