...
\tcr{are you sure these are HCSE that they start with? }
...
\tcr{a path of what, for what? sentence feels incomplete} \cite{dlm}. 
...
for HNCSE is also valid for HCSE under certain conditions \tcr{like what? talk}: this remark is of importance since it will guarantee, from a 
...
equation for Euler equations (see \tcr{Equation~(abc)} in \cite{Marco_paper_low_mach}). $\mathcal{R}_1$ is known to be positive when 
...
such as $ \partial_d \alpha_k = 0$ and $\partial_{dd} \alpha_k \leq 0$ for $1\le d \le {\tt dim}$. \tcr{check} 
...
\tcr{did you update this. I thought a $q$ term was missing according to a reviewer...?!?}
...
thus possibly enabling the dissimnation of multi-phase flows using such tools.} 
...
%
\tcr{the SGEOS was used TWICE before and I think this is the only time it is given correctly. Do we even need to give it before?}
...
\end{align}
%
\tcr{so is $ e_k -q_k = C_{v,k} T_k$ or is a $q_k$ missing in the second eq.?}
...
%-----------------------------------------------------------
%
In this example, we set the relaxation coefficients $\mu_P$ and $\lambda_u$ to large values; the pressures and velocities in each phase should become identical, \tcr{as expected by performing a Chapman-Enskog expansion}. 
...
for the above cases (1) and (2). \tcr{I was going to say ``not true, the figures below do not show cases 1 and 2'', because I can barely notice the light blue. Please you another color!}
...
\tcr{this is only theoretical since we do not show numerical values; should we keep on emphasizing this?}, 
convergence of the numerical solution to an entropy solution \tcr{for concave physical phasic entropy functions $s_k$ {\it is this needed in the conclusions, sounds heavy}}, 
