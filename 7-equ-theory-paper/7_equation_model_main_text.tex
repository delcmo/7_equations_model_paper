...
\cite{jlg,Marco_paper_low_mach} (for finite volume, continuous Galerkin, and spectral FEM discretizations) and to \cite{valentin} 
...
employed for the (single-phase) Euler equations \cite{jlg,Marco_paper_low_mach}. The method consists in adding dissipative terms to the system of equations under 
...
single-phase viscous regularization of \cite{jlg,Marco_paper_low_mach}. We will later provide a possible definition for $\mbold l_k$.
...
\end{equation}
%
Following Lemma 3.1 of \cite{jlg} and using \eqt{eq:reg-euler-qu}, a nonnegative density principle can be proved for the quantity 
$\hat{\rho}_k$. Thus, we conclude that the quantity $\alpha_k \rho_k$ remains nonnegative under the assumption $\beta_k = \kappa_k$. 
Note that unlike Lemma 3.2 of \cite{jlg}, the density 
...
the viscous regularization derived for Euler equations \cite{jlg,Marco_paper_low_mach} in the limit $\alpha_k \to 0,1$. The viscous 
