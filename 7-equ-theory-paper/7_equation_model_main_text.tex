...
and theoretical importance: the multi-phase low-Mach regime and the multi-phase \tcr{thermodynamical?/mechanical?} \tcb{This is for Ray to answer} equilibrium treated as a Chapman-Enskog expansion.
...
for HNCSE remains valid for HCSE \tcb{under the condition that the nonconservative form may be written under a conservative form}
...
Sounds weird}\tcb{see paragraph above equation 2.7 on page 5 of [20]}: this remark is of importance since it will guarantee, from a 
...
\tcr{can you check these expressions ofr f, g, h. For example, the aread seems to be missing from f and g. Alpha is also missing. And f does not even depend on l. A lot of things are missing. This is very strange.} \tcb{Just fixed it. I got confused between $\tilde{h}$ and $h$}.
...
the left-hand side of \eqt{eq:ent-res-7-eqn-diss-terms3} is non-negative if and only if the phasic entropy function $s_k$ is concave with respect to $e_k$ and $\rho_k^{-1}$ and the following expressions  
\begin{align}
& \mbold f_k   =  \kappa_k \grad \rho_k
  \\
&  \mathbb{g}_k = \alpha_k \rho_k A \mu_k \mathbb{F}(\mbold u_k) +  \mbold f_k \otimes \mbold u_k
  \\
&  \mbold h_k =   \alpha A \kappa_k \grad (\rho e)_k - \tfrac{1}{2}\| \mbold u_k\|^2  \mbold f_k + (\rho e)_k \mbold l_k,
\end{align}
...
at an entropy extremum, the inequality $\alpha_k \rho_k \frac{D s_k}{Dt} \geq 0$, holds {\it independently} of the definition of the dissipative term $\mbold l_k$ utilized in the volume fraction equation. \tcr{check my mods on that last sentence} \tcb{I modified it: $\alpha_k \rho_k \geq 0$ is not needed}
%
%----------------------------------------------------------------------------------------------------------------------------
...
thus possibly enabling the dissemination of multi-phase flow simulations using such tools.} \tcb{ok}
