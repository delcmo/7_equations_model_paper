...
%
The \tcr{terms underlined in \eqts{eq:sev_equ-with-diss-terms} are part of the inviscid SEM model} \tcb{done}, see the unregularized system of equations (\eqref{eq:liq-7-eqn-sect5_no_exchange}); these terms are entropy-producing and we refer the reader to \app{app:sev-equ-model-entropy} for a detailed derivation of the 
...
(a) the isentropic low-Mach limit where the seven-equation model \tcr{degenerates\footnote{negative connotation!}} \tcb{I am not sure if we can say it in a different way} to an incompressible system of 
...
\tcr{In the low-Mach limit, the two phases are assumed not to interact and, thus, the volume fraction 
of each phase remains constant}. \tcb{ok} 
...
that the micro and macro scale interaction effects are neglected}. \tcb{ok}% \tcb{I added a comment at the beginning of this section} 
...
In the second test, a low-Mach flow problem is considered.
...
\end{figure}
%
%\tcr{could we change the legend in the plots so that it is more explicit. I'd say type 1 should be replaced with LLF, type 2 with LLF, $\beta_k=0$, type
%3 with  LLF, $\beta_k=\kappa_k0$. Matlab takes latex style in its legend function\\}
...
%
%----------------------------------------------------------
\subsection{Low-Mach flow test case}\label{sec:second-test}
%-----------------------------------------------------------
%
...
\cite{Marco_paper_low_mach} for a flow where the relaxation coefficients $\mu_P$ and $\lambda_u$ are set to zero (independent phases).
%
The variable area expression for the nozzle is given by $A(x) = 1 + \tfrac{1}{2} \cos(2 \pi x / L)$ with length $L=1\ m$. 
At the inlet, the stagnation pressure and temperature are set to $P_0 = 1 MPa$ and $T_0 = 453 K$, respectively. 
At the outlet, only the static pressure is specified: $P_s = 0.5 MPa$. 
...
% and remains constant until steady-state is reached.
The stiffened gas equation of state is used to model the liquid water and steam phases \tcr{how about the vapor?} \tcb{done} with the parameters provided in 
...
the liquid-phase simulation experiences a low-Mach flow. 
%
To illustrate the results of the low-Mach asymptotic study we consider two cases. 
%
In case (i), we run the test with the same phasic viscosity 
coefficients as in \sct{sec:first-test}: $\mu_k =  \kappa_k = \tfrac{h}{2} \left( ||\mbold u_k|| + c_k \right)$. Using the non-dimensional 
numbers defined in \eqt{eq:norm_param} along with the above definitions of the phasic viscosity coefficients, it is found that the scaling of the phasic 
numerical Reynolds and P\'eclet numbers is a function of the phasic Mach number as follows: $\Re_k=\Pe_k^\mu = M_k / (1+M_k)$ (note that the P\'eclet numbers 
defined for the volume fraction equation is not defined since the volume fraction equation is not needed). Such a scaling will efficiently 
stabilize the steam phase flow that becomes supersonic and develops a steady-state shock ($M_k \sim 1 $). For the liquid phase that reaches 
a low-Mach steady-state flow, however, the artificial dissipative terms will be ill-scaled and should not reach the correct steady-state 
solution. 
%
In case (ii), we propose a fix to the ill-scaling effect observed in the liquid phase. From the low-Mach asymptotic study presented 
in \cite{Marco_paper_low_mach}, it is highlighted that the low-Mach asymptotic limit is recovered when the phasic numerical numbers scale 
as one. Thus, a simple fix to the ill-scaling of the dissipative terms will be to multiply the phasic viscosity coefficients by the phasic 
Mach number such as:  $\mu_k =  \kappa_k = M_k \tfrac{h}{2} \left( ||\mbold u_k|| + c_k \right)$. Note that in the supersonic case, the 
efficiency of the numerical method will be not altered since $M_k \sim 1$. 

The steady-state numerical solutions for water liquid and steam phases are presented in \fig{fig:vap-phase} and \fig{fig:liq-phase}, respectively, for the above cases (i) and (ii).
%
\begin{figure}[H]
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
...
In case (i), the steady-state numerical solution does not match the exact solution and illustrates the ill-scaling effect of the 
dissipative terms for low-Mach flows (see Mach plot in  \fig{fig:liq-phase-mach}). When run with the fix that corresponds 
to case (ii), i.e. the viscosity coefficients are multiplied by the local Mach number, the steady-state numerical solution 
converges to the exact solution as observed in \fig{fig:liq-phase}. The dissipative terms are no longer ill-scaled and the 
proper low-Mach asymptotic limit is recovered. 
