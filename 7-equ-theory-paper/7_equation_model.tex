...
%
The \tcr{terms underlined in \eqts{eq:sev_equ-with-diss-terms} are part of the inviscid SEM model} \tcb{ok}, see the unregularized system of equations (\eqref{eq:liq-7-eqn-sect5_no_exchange}); these terms are entropy-producing and we refer the reader to \app{app:sev-equ-model-entropy} for a detailed derivation of the 
...
(a) the isentropic low-Mach limit where the seven-equation model \tcr{degenerates\footnote{negative connotation!}} \tcb{I have always heard Ray use this word. We should ask him.} to an incompressible system of 
...
of each phase remains constant}. \tcb{In the low-Mach limit and for the purpose of this section, the two phases are assumed not to interact and, thus, the volume fraction 
...
that the micro and macro scale interaction effects are neglected}. \tcb{ok}
...
%
\tcr{I think the text in \underline{all} of the figures (legend+axis labels) is a bit too small for a journal} \tcb{I will issue new plots. We can still give the paper to Ray for review}
\begin{figure}[H]
        \centering
...
The stiffened gas equation of state is used to model the liquid and vapor with the parameters provided in \tbl{tbl:stff_gas_eos}.
%
\begin{table}[!htbp]
\begin{center}
...
\end{table}
%
\tcr{if i am not mistaken, we have 2 sets of parameters for the liquid in the low-mach paper, so the reader will have no idea 
which set of parameters has been used here. why don't you copy table 2 from the low mach paper and remove the line that doesn't apply here} \tcb{done}
%
This test case is of interest for multiple reasons: (a) a steady state is reached, (b) 
an analytical solution is available \cite{nozzle_exact} \tcr{add reference to analytical solution paper here} \tcb{done}, (c) due to the different compressibilities between the phases,
the vapor phase will become supersonic and will develop a steady shock while the liquid phase will be low-Mach flow ($M_\text{liquid} \approx 10^{-2}$). 
%
We illustrate the low-Mach asymptotic study of \sct{sec:low-Mach} with two cases. 
\tcr{I know beta is not needed here, but I prefer to leave it to avoid questions later. what do you think?\\} \tcb{I added a line below}
%
...
$\mu_k =  \kappa_k = \beta_k = \tfrac{h}{2} \left( ||\mbold u_k|| + c_k \right)$ \tcb{(note that the artificial viscosity flux function of the phasic viscosity coefficient $\beta_k$ is not active because the volume fraction remains constant)}. Using the non-dimensionalized variables defined in \eqt{eq:norm_param} \tcr{wrong eq, should be 36} \tcb{done}, along with the above definitions of the phasic viscosity coefficients, it is found that the scaling of the phasic 
numerical Reynolds and P\'eclet numbers defined in \eqt{eq:ref_numb}, is a function of the phasic Mach number as follows: $\Re_k=\Pe_k^\kappa=\Pe_k^\beta = 2M_k / (1+M_k)$. 
...
(see Mach plot in \fig{fig:vap-phase-mach}). The numerical and exact solutions perfectly match but in the vicinity of the shock where smoothing occurs due to the over-dissipative nature of the local Lax-Friedrichs scheme. 
%
\begin{figure}[H]
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
