...
(a) the isentropic low-Mach limit where the seven-equation model \tcr{degenerates\footnote{negative connotation!}} to an incompressible system of 
...
of each phase remains constant}. 
...
The stiffened gas equation of state is used to model the liquid water \tcr{how about the vapor?} with the parameters provided in 
...
numbers defined in \eqt{eq:norm_param} along with the above definitions of the phasic viscosity coefficients, it is found that the phasic 
numerical Reynolds and P\'eclet numbers scale as the phasic Mach number: $\Re_k=\Pe_k^\mu = M_k / (1+M_k)$ (note that the P\'eclet numbers 
...
stabilize the steam phase that develops a steady-state shock and thus, becomes supersonic ($M_k \sim 1 $). For the liquid phase that reaches 
...
The steady-state numerical solutions for the above cases (i) and (ii) of two vapor and liquid phases, are presented in \fig{fig:vap-phase} and \fig{fig:liq-phase}, respectively. 
%
\begin{figure}[H]
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
...
\end{figure}
%
As shown in \fig{fig:liq-phase}, the effect of the fix is more visible on the numerical solution for the liquid phase. 
In case (i), the stead-state numerical solution does not match the exact solution and illustrates the ill-scaling of the 
