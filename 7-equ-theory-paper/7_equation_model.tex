...
of Mach numbers (subsonic, transonic and supersonic flows) and is the topic of \sct{sec:low-Mach}. But first, we investigate the effect 
...
The numerical illustration consists of a 1-D shock tube \tcr{of length 1} separated by a membrane \tcr{placed at $x=0.5$}. The initial left/right values of the pressures are
 $(P_{left}=10^6)$ and $(P_{right}=10^5)$. The initial density and volume fraction are uniform and set to $\rho_1=10$, $\rho_2=1$ and $\alpha = 0.5$, respectively. Both fluids are initially at rest. 
\tcr{it's a math paper. can't we use Pleft/right = 10 and 1 instead of super higher values?}\\
%
...
Following Guermond et al. (Section 2.2 in \cite{jlg}), we employ $\mu_k =  \kappa_k = \tfrac{1}{2} h \left( ||\mbold u_k|| + c_k \right)$ where $h$ is the grid size and $||\mbold u_k|| + c_k$ is the 
largest eigenvalue of the phasic Euler equations. 
...
At $t=0$, the membrane is suddenly removed and the simulation is ran until $t=.0305\times 10^{-3}$; see \fig{fig:two-phase}.
The viscous numerical solutions do not display any instability in the vicinity of the shock region ($x \approx 0.63$). In \fig{fig:two-phase-density}, the contact wave is smeared 
...
\tcr{would it be possible to run this with no viscosity? Or ran it with no viscosity in the density equations, just to compare, as Guermond did?}\\
...
%The chosen definitions for the viscosity coefficients 
%because of the over-dissipative nature of the viscosity coefficients. On the other hand, the discontinuities will be smeared
%but there are no undershoots or overshoots. 
%Numerical results are presented inat the final time $t=305 \ \mu s$ and were obtained using a Continuous Galerkine Finite Element method along with the implicit temporal integrator BDF2 \cite{bdf2} within the Moose framework \cite{Moose}.
%
\begin{figure}[H]
        \centering
...
\end{figure}
%
We have also verified that if we set the relaxation coefficients $\mu_P$ and $\lambda_u$ to zero (which is equivalent to solving Euler equations for each fluid independently), we recover 
the single-phase Euler shock tube results recently published in \cite{jlg1, jlg2} using the same viscous stabilization. 
...
artificial viscosity coefficients borrowed from the Lax-Friedrichs scheme.

As future work, this two-phase flow viscous regularization should be tested with high-order, less-dissipative, artificial viscosity schemes, such as the entropy viscosity method (e.g., see \cite{jlg1} for single-phase supersonic flows and \cite{Marco_paper_low_mach} single-phase subsonic and transonic flows).
