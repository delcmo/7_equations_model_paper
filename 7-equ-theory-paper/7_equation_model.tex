...
%---------------------------------------------------------------------------------
%
%We now investigate under which conditions on the dissipative fluxes $\mbold f_k$, $\mathbb{g}_k$ and $\mbold h_k$, the entropy minimum principle holds for the system of equations given in \eqt{eq:sev_equ-with-diss-terms}. First, the right-hand side of \eqt{eq:sev_equ-with-diss-terms} is recast as a sum of a quadratic term and other positive terms in \lem{lem:evm}. Then, the entropy minimum principle is shown to hold in \theo{the:evm}.
%%
%\begin{lemma}\label{lem:evm}
%Setting
%%
%\begin{subequations}
%\begin{align}
%  \mbold f_k   &= \kappa_k \grad( \rho)_k +\rho_k \mbold  l_k
%  \\
%  \mathbb{g}_k -  \mbold f_k \otimes \mbold u_k &= \alpha_k \rho_k A \mu_k \mathbb{F}(\mbold u_k) 
%  \\
%  \mbold h_k + \tfrac{1}{2}\| \mbold u_k\|^2  \mbold f_k &= \kappa_k \grad(\rho e)_k + (\rho e)_k \mbold l_k,
%\end{align}
%\end{subequations}
%%
%the right-hand side of \eqt{eq:ent-res-7-eqn-diss-terms_rearranged} can be recast as a function of a quadratic form $Q_k = \mathbf{X}^T_k \mathbb{\Sigma}_k \mathbf{X}_k$ and other terms as follows:
%%
%\begin{multline}
%\alpha_k \rho_k A \frac{Ds_k}{Dt} =  \mbold f_k \cdot \grad s_k + \div \left( \alpha_k A \rho_k \kappa_k  \grad s_k \right)  \\
%- \alpha_k \rho_k A \kappa_k Q_k + (s_e)_k \alpha_k A \rho_k \mu_k \grad^s \mathbb{F}(\mbold u_k)  : \grad \mbold u_k
%\end{multline}
%%
%\end{lemma}
%%
...
%\end{proof}
%%
%\begin{theorem}{(Entropy Minimum Principle)}\label{the:evm}
%Assume that the solution to the system of equations given in \eqt{eq:sev_equ-with-diss-terms} is smooth and that the initial conditions of the internal energy and the density are constant outside some compact set. Lastly, assume that \eqt{eq:ent-res-7-eqn-diss-terms3} holds, then the minimum entropy principle holds for the SEM,
%%
%\begin{equation}
...
%\end{subequations}
%%
%However, the definition of $\mbold l_k$ proposed in \eqt{eq:def-l-k-wrong-sect4} was not considered as valid for the following reasons: positivity of the volume fraction cannot be achieved \tcr{can you give ME some details as of why in my office one day?} and the parabolic regularization is not recovered when all viscosity coefficients are set equal. \tcb{I probably told you that the with the above definition of $l_k$, the sign of the entropy residual associated to the volume fraction equation cannot be determined. }
%\end{remark}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
the two phases do not interact. Thus, the seven-equation model degenerates to two sets of Euler equations with a pseudo cross-sectional area $\alpha_k A$. It is also devised from the volume fraction equation \eqt{eq:sev_equ-parab-vf} that the volume fraction $\alpha_k$ is time independent. 
...
\end{equation}
%
%Because we consider that phases do not interact with each other, it is assumed that the interfacial pressure and velocity scale as the phasic pressure and velocity, respectively: $P_{int,\infty} = \rho_{k,\infty} c^2_{k,\infty}$ and $u_{int,\infty} = u_{k,\infty}$. 
...
+ \frac{1}{\Pe_{k,\infty}^\beta } \grad^*\cdot \left( \rho_k e_k \alpha_k A \beta_k \grad \alpha_k \right)^*
\end{multline}
%
\end{subequations}
%
...
In this case and assuming that $\alpha_k \neq 0$, we have
%
...
\partial_t(\alpha_k A  \rho_k E_k) +  \div \left(\alpha_k A \mbold u_k \rho_k E_k\right)_0 + \alpha_{k,0} \div \ \left( A \mbold u_k P_k \right)_0  = \\
 \div \left[A \alpha_k \kappa_k \grad\left(\rho_k e_k\right) \right] 
\end{multline}
%
\end{subequations}
%
