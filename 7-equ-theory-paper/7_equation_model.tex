...
The numerical illustration consists of a 1-D shock tube of length 1 separated by a membrane placed at $x=0.5$. The initial left/right values of the pressures are
 $(P_{left}=10)$ and $(P_{right}=1)$. The initial density and volume fraction are uniform and set to $\rho_1=10$, $\rho_2=1$ and $\alpha = 0.5$, respectively. Both fluids are initially at rest. 
...
largest eigenvalue of the phase $k$. Under this assumption, the numerical scheme is equivalent to Lax-Friedrich scheme which is known to be over-dissipative.
...
The viscous numerical solutions do not display any instability in the vicinity of the shock region ($x \approx 0.73$). In \fig{fig:two-phase-density}, the contact wave is smeared 
...
\end{figure}
%
We further investigate the influence of the viscosity coefficients onto the numerical simulations by running the same test as above for three different cases:
(1) all viscosity coefficients are equal which corresponds to Lax-Friedrich scheme, (2) the definition of the viscosity coefficients $\mu_k$ and $\kappa_k$ are unchanged and $\beta_k$ is set to zero, and (3) the viscosity coefficients $\kappa_k$ and $\beta_k$ are set to zero and $\mu_k$ is unchanged. Numerical results of the density profiles are presented in \fig{fig:density}
%
\begin{figure}[H]
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
...
\end{figure}
%
In case (1) and (2), the numerical solutions does not display any instability in the density profiles. Even when setting the volume fraction viscous coefficient $\beta_k$ to zero, the density profiles remains smooth. However, in case (2) the volume fraction profiles displayed spurious oscillations in the vicinity of the contact region (not shown here). In the third case, the phasic continuity equations are no longer regularized since $\kappa_k=0$ by assumption, leading to the formation of instabilities in the pre-contact region and the presence of an undershoot in the post-contact region. This result is consistent with the study performed by Guermond et al. in \cite{jlg} for Euler equations that compared numerical solutions obtained when using a viscous regularization involving a dissipative term in the continuity equation and the Navier-Stokes regularization.
