...
that they select the same entropy solution (\tcr{this can be demonstrated for Burgers equations, see \cite{Evans1998}}) \tcb{ok}.
...
\end{multline}
%
The \tcr{underlined} terms in \eqt{eq:rho-7-eqn-model-sect4} and \eqt{eq:int-ener-7-eqn-model-sect4}, also present in the derivation of 
...
We remind the reader that the $\grad \alpha_k$ is assumed zero for the purpose of the low-Mach asymptotic study, and begin by expanding each variable in powers of the Mach number. As an example, the expansion for the pressure is given by:
%
\begin{equation}
\label{eq:expansion}
...
\end{equation}
%
\tcr{remind the reader of the beginning of section 4.1} \tcb{done}
\begin{multline}
...
\tcr{recall what the model is: eq10+regular of eq.25}\tcb{you want to do it here and below as well}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Numerical illustrations}\label{sec:num-ill}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\tcr{which equations??? Eq. 10 + regularization defined in Eq. 25. when malbda and mu are large, then it is equivalent to the 5-eq model} \tcb{done}


%
We finish with a numerical example to illustrate the ability of the proposed viscous regularization (\eqt{eq:visc-reg-7-equ-sect4}) to stabilize the SEM two-phase flow equations (\eqt{eq:liq-7-eqn-sect5}) in two tests.
The aim of the first numerical test, is to demonstrate that the viscosity regularization prevents undershoots and overshoots near discontinuities (\sct{sec:first-test}). The purpose of the second test is to illustrate the low-Mach asymptotic limit performed in \sct{sec:low-Mach} and highlights effects of ill-scaled dissipative terms when simulating low-Mach flows (\sct{sec:second-test}).
%----------------------------------------------------------
