...
\eqt{eq:ent-res-7-eqn-diss-terms_rearranged} can be further simplified by defining the viscous fluxes $\tilde{\mbold f}_k$ and $\tilde{\mbold h}_k$ and a viscous tensor \tcr{check} $\mathbb{F}(\mbold u_k)$ as a function of the dissipative terms $\mbold f_k$, $\mathbb{g}_k$, $\mbold h_k$ and $\mbold l_k$ as follows:
%
...
\end{align}
\end{subequations}
\tcr{ the : operator in the last equation is not consistent with equation 10d, I think this is wrong\\}
%
...
The numerical Reynolds and P\'eclet numbers are related to the phasic entropy \tcr{how can we justify using that term here?}
...
terms, as is the case with the entropy \tcr{here again} viscosity method, the main properties of the low-Mach asymptotic limit must be preserved.
...
\tcr{how did A come out of the left-hand side spatial derivative to be placed on the denominator of the RHS?\\}
\begin{multline}
