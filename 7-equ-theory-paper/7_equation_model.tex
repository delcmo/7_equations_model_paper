...
theoretical limit-cases. \tcr{ (I want to convey the idea that we do a theoretical analysis, but show no numerical results. not sure this is the best wording)} \tcb{done} First, a Chapman-Enskog expansion is performed for the regularized seven-equation two-phase flow model and we show that the five-equation flow 
model of Kapila is recovered with a well-scaled viscous regularization. This limit-case is illustrated with numerical results for a 1-D shock tube. Second, a low-Mach asymptotic limit of the regularized seven-equation flow model is carried out, yielding an incompressible two-phase flow model. \tcr{yielding blabla} \tcb{done}
\end{abstract}
%-------------------------
%-------------------------
\begin{keyword}
...
globally smooth solution may not occur because of the nonlinearity of the flux functions and other nonlinear terms.\tcb{this one is a pain} The concept of a weak
...
unique solution, the ``entropy solution,'' among the infinitely many weak solutions. Although there are
several different ways of defining the entropy condition, it is generally hoped that they are all equivalent in the sense that they select \tcr{the same entropy 
solution}. \tcr{I need to check with Bojan at lunch} 
For numerical schemes, the entropy condition and solution are sought through utilization of so-called conservative formulations of the 
physically descriptive equations, along with the addition of an appropriately specified physical or artificial viscosity, either added directly to the governing 
...
\tcr{nothing is said about the Chapman-Enskog study here, yet we ``advertize'' this in the abstract. It feels unbalanced...} \tcb{done}
...
model of Kapila \cite{Kapila_2001} through a Chapman-Enskog expansion \cite{dellacherie, GuillardMurrone2003}. The objective is to show 
...
well-scaled dissipative terms over a wide range of Mach numbers. Our theoretical approach is illustrated in \sct{sec:num-ill} with numerical results 
...
\end{subequations}
%
\tcr{is it correct that the averaged quantities work at the micro level? Just by looking at the name, I would have thought that average quantities
work at the macro level.\\} \tcb{ I think Ray added this part but I would agree with you}
...
The next step consists in deriving the entropy equation for the phase $k$, on the same model as what was done in \app{app:sev-equ-model-entropy} but 
...
the entropy reaches its spatial minimum, the entropy minimum principle holds for each phase $k$.
...
next paragraph, the effect of the viscous regularization in the case where the SEM degenerates to the five-equation model of 
...
terms for all-Mach flows (subsonic, transonic and supersonic flows). First, the scaled SEM are derived. Then, two limit cases will be considered to determine 
...
(the volume fraction equation is omitted since the volume fraction is assumed constant for each phase): % and there is not relaxation terms):
% 
...
and the leading-order expressions for the continuity, momentum and energy equations are:
\begin{subequations}
...
artificial viscosity coefficients borrowed from the local Lax-Friedrichs scheme. We also analyzed the effect of removing stabilization from the volume fraction and the continuity equations.
