...
The Seven-Equation two-phase flow Model employed in this paper is obtained by assuming that each phase satisfies the single-phase Euler equations (with phase-exchange terms) and by integrating the latter over a control volume after multiplication by a phasic characteristic function. The detailed derivation of the governing equations for a phase $k$ in interaction with a phase $j$ can be found in \cite{SEM}. In the SEM, each phase obeys a mass, a momentum and an energy balance equation, supplemented by a non-conservative equation for the volume fraction as shown in \eqt{eq:liq-7-eqn-sect5}.
%
...
\end{subequations}
%
where $\alpha_k$, $\rho_k$, $\mbold u_k$ and $E_k$ denote the volume fraction, the density, the velocity vector and the total specific energy of phase $k$, respectively. The volume fraction, mass, momentum and energy exchange terms from phase $k$ to phase $j$ are denoted by the symbols $\mathbb{S}_{k \to j}$, $\Gammakj$, $M_{k \to j}$ and $E_{k \to j}$, respectively, are assumed to be determined by the methods of rational thermodynamic \cite{BaerNunziato, PassmanNunziato} and thus will only yield positive terms in the right-hand side of the entropy inequality equation. They also obey to the following closure relations:
%
...
\end{equation}
%
From a physical point of view, \eqt{eq:tot-ent-res-sct4} state that the total entropy of the system increases as a function of time. From a numerical perspective, the pressure and velocity relaxation terms add dissipation to the system of equations (see \eqt{eq:ent-eqn-7-eqn-model}).
...
\eqt{eq:ent-res-7-eqn-diss-terms_rearranged} can be further simplified by defining the dissipative terms $\tilde{\mbold f}_k$ and $\tilde{\mbold h}_k$ and a function $\mathbb{F}(\mbold u_k)$ as a function of the dissipative terms $\mbold f_k$, $\mathbb{g}_k$, $\mbold h_k$ and $\mbold l_k$ as follows:
%
...
\end{subequations}
%
where $\mu_k$ is a positive viscosity coefficient for phase $k$. The functional form of the dissipative terms defined in \eqt{eq:def-diss-terms-sect4} will be derived further as it is the purposed of this section. Substituting the expressions of \eqt{eq:def-diss-terms-sect4} into \eqt{eq:ent-res-7-eqn-diss-terms_rearranged} yields:
%
...
\end{enumerate}
%
At this point in the paper, we have derived a viscous regularization for the Seven-Equation two-phase flow Model that ensures positivity of the entropy residual, uniqueness of the numerical solution when assuming concavity of the phasic entropy $s_k$, and is consistent with the viscous regularization derived for Euler equations \cite{jlg, Marco_paper_low_mach} in the limit $\alpha_k \to 1$. The viscous regularization involves a set of three viscosity coefficients for each phase, $\mu_k$, $\kappa_k$ and $\beta_k$, that are assumed positive. Definition of the viscosity coefficients should be devised from the scaled SEM in order to ensure well-scaled dissipative terms for a wide range of Mach numbers (subsonic, transonic and supersonic flows) and is the topic of \sct{sec:low-Mach}. Also, effect of the viscous regularization has to be investigated in the case where the SEM degenerates to the 5-equation model of Kapilla (REF) which is the purpose of next section.
%
%---------------------------------------------------------------------------------
\subsection{A Chapman-Enskog expansion of the regularized Seven-equation two-phase flow model}
%---------------------------------------------------------------------------------
%
The five-equation two-phase flow model of Kapilla (REF) is obtained from the non-regularized SEM by performing a Chapman-Enskog expansion. The steps of the derivation are well detailed in the literature and can be found in \cite{dellacherie}. The objective of this section is to perform a Chapman-Enskog expansion of the \emph{regularized} SEM derived in \sct{sct:end-min-pr} and investigate the behavior of the dissipative terms: we want to make sure  that the dissipative terms remain well-scaled and can efficiently stabilize the resulting system of equations. Only the main results of the derivation will be given in this section and were obtained by following the same steps as in \cite{dellacherie}. First, the pressure and velocity relaxation coefficients are scaled by a small coefficient $\epsilon$ that describes the strength of the perturbation: $\mu_P \to \frac{\mu_P}{\epsilon}$ and $\lambda_u \to \frac{\lambda_u}{\epsilon}$. Then, each variable of interest is explicitly expanded in power of $\epsilon$ as shown in \eqt{eq:P-expansion-epsilon} and the leading and first-order equations are determined.
%
\begin{equation}\label{eq:P-expansion-epsilon}
P=P_0+\epsilon P_1+\epsilon^2 P_2 + \dots
\end{equation}
%
The dissipative terms derived in \sct{sct:end-min-pr} are not a function of the relaxation coefficients $\mu_p$ and $\lambda_u$ and thus, will have the same scaling as the advection flux. From the leading-order momentum and energy equations, the leading-order phasic pressure and velocity are proved equal to each other such that $P_{k,0}=P_{j,0}=P_0$ and $\mbold u _{k,0} = \mbold u_{j,0} = \mbold u_0$. Using this previous result, the first-order system of equations yield the regularized five-equation model of Kapilla recalled in \eqt{eq:liq-5-eqn} where we defined $\rho u = \sum_{i=k,j} \alpha_i \rho_i u_i$ and $\rho E = \sum_{i=k,j} \alpha_i \rho_i E_i$ as the mixture momentum and energy, respectively.
%
...
\begin{align}
  ...
  &= A K \div \mbold u_0 + \div \left( \beta_k \grad \alpha_k \right)_0 \,,
\end{align}
\begin{align}
  % liquid mass conservation
...
\end{subequations}
%
The mixture pressure is defined as $P= \sum_{i=k,j} \alpha_i P_i$ and is a function of the phasic pressures that are computed from the Stiffened gas equation of state. The notation $(fg)_0$ means that we only keep the $0^{th}$ order terms in the product $f g$. The function denoted by $K$ in \eqt{eqn:multi-d-5-eqn-liq-vol} is computed from \eqt{eq:K-fnct} and was derived from the Chapman-Enskog expansion. 
%
\begin{equation}\label{eq:K-fnct}
K=
\end{equation}
%
The main conclusion to draw from this study, is that the leading-order equations are not modified by the addition of dissipative terms to the equations: the phasic pressure and velocity remain equal to each other in first approximation. Also, from \eqt{eq:liq-5-eqn}, it is noted that the dissipative terms in the right hand side are well scaled and, thus, should be able to efficiently stabilize the numerical solution assuming that the viscosity coefficients are properly defined for a wide range of Mach numbers which is investigated next.  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A low-Mach asymptotic limit needs to be performed on the regularized system of equations in order to properly scale the viscosity coefficients and to recover the incompressible asymptotic equations \cite{LowMach1, LowMach2, LowMach3}. The purpose of this section is to derive the scaled SEM and investigate the scaling of the dissipative terms to ensure well-scaled dissipative terms for all-Mach flows (subsonic, transonic and supersonic flows). First, the scaled SEM are derived. Then, two limit cases will be considered to determine appropriate scaling for the entropy 
...
A \grad \left( \alpha_k P_k \right)_0 = 0 \Rightarrow \grad P_{k,0} = 0
\end{equation}
%
and at order $M_{k,\infty}^{-1}$
\begin{equation}
\label{eq:asympt_equ1_mom}
A \grad \left( \alpha_k P_k \right)_1 = 0 \Rightarrow\grad P_{k,1} = 0 \, .
\end{equation}
\end{subequations}
%
...
\end{equation}
%
That is, the leading-order of the product of velocity and cross section is divergence-free, which reduces to the standard divergence-free velocity field for a constant area $A$ and volume fraction $\alpha_k$, $\div \mbold u_{k,0}=0$. Finally, recall that \eqt{eq:div_free_energy} was written for the stiffened gas law; to retrieve standard result for the ideal gas law, simply set $P_{k,\infty}=0$. For the ideal gas law with a constant cross section, the previous result can be expressed as
%
\begin{equation}
...
have $\frac{\grad \left( \alpha_k P_k \right)}{M_{k,\infty}^2}=\grad \left( \alpha_k P_k \right)_2$ and therefore the pressure gradient term may need to be stabilized by 
