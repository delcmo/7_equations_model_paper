...
%
%---------------------------------------------------------------------------------
\subsection{Scaling of the viscous regularization in the five-equation asymptotic limit of the seven-equation two-phase flow model}
%---------------------------------------------------------------------------------
%
The Seven-Equation two-phase flow Model is known to degenerate into the five-equation two-phase flow model of Kapilla when performing a first-order Chapman-Enskog expansion (REF). The objective of this section is to investigate the behavior of the dissipative terms derived in \sct{sct:end-min-pr}, in the asymptotic limit. When performing a Chapman-Enskog expansion, the scale of the relaxation coefficients, $\mu_P$ and $\lambda_u$, is made explicit by using a small coefficient $\epsilon$ and each variable is expanded in powers of $\epsilon$ as follows:
%
\begin{equation}
P=P_0+\epsilon P_1+\epsilon^2 P_2 + \dots
\end{equation}
%
The dissipative terms derived in \sct{sct:end-min-pr} are not a function of the relaxation coefficients $\mu_p$ and $\lambda_u$ and thus, will have the same scaling as the advection flux. From the leading-order equations obtained with a Chepman-Enksdog expansion, the leading-order phasic pressure and velocity are proved equal such that $P_{k,0}=P_{j,0}=P$ and $\mbold u _{k,0} = \mbold u_{j,0} = \mbold u$. Using the previous result, the first-order system of equations yield the five-equation model of Kapilla recalled in \eqt{eq:liq-5-eqn}: 
%
\begin{subequations}\label{eq:liq-5-eqn}
\begin{align}
  % liquid volume fraction
  \label{eqn:multi-d-5-eqn-liq-vol}
  \frac{\partial \alpha_{k} A}{\partial t} + A\mbold u \cdot \grad \alpha_{k}
  &= A K \div \mbold u + \div \left( \beta_k \grad \alpha_k \right) \,,
\end{align}
\begin{align}
  % liquid mass conservation
  \label{multi-d-5-equ-liq}
  \frac{\partial \left( \alpha \rho \right)_{k} A}{\partial t}
  + \div \left[ (\alpha \rho)_k \mbold u A\right]
  &= -\Gammakj + \div \mbold f_k\,,
\end{align}
\begin{align}
  % vapor mass conservation
  \label{multi-d-5-equ-vap}
  \frac{\partial \left( \alpha \rho \right)_{j} A}{\partial t}
  + \div \left[ (\alpha \rho)_j \mbold u A\right]
  &= \Gammakj + \div \mbold f_j \,,
\end{align}
\begin{align}
  % liquid momentum
  \frac{\partial \rho \mbold u A}{\partial t}
  + \div \left[ A \left( \rho \mbold u \otimes \mbold u + P \mathbb{I} \right) \right]
  = P \grad A + \sum_{i=k,j} \div \mathbb{g}_i\,,
\end{align}
\begin{align}
  % liquid total energy
  \frac{\partial \rho E A}{\partial t}
  + \div \left[ \mbold u A \left( \rho E + P \right) \right]
  = \sum_{i=k,j} \div \left( \mbold h_i + \mathbb{g}_i : \mbold u \right) \,,
\end{align}
\end{subequations}
%
where the $\rho u = \sum_{i=k,j} \alpha_i \rho_i u_i$ is the mixture momentum and $\rho E = \sum_{i=k,j} \alpha_i \rho_i E_i$ is the mixture energy. The mixture pressure is defined as $P= \sum_{i=k,j} \alpha_i P_i$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
