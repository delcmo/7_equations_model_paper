...
(1) all viscosity coefficients are equal (Lax-Friedrichs scheme already shown in \fig{fig:two-phase}) \tcr{check} \tcb{good}, 
(2) the definitions for $\mu_k$ and $\kappa_k$ are unchanged but $\beta_k$ is set to zero \tcr{the figure shows no effect. someone may conclude we do not need to stabilize the volume fraction. do you have a plot of another quantity where we see an effect?}, 
