...
After briefly detailing the spatial and temporal discretization methods, numerical results of flows developing pressure waves taken from the published literature and obtained with the system code RELAP-7 are presented in \sct{sec:results}. A set of shock tube tests \cite{Sokolowski-Koszela} are run to assess the robustness and the accuracy of the EVM. Water hammer-like tests are also investigated \cite{Serre-bestion}.
%
...
\end{subequations}
%
where the function $sgn(x)$ returns the sign of the variable $x$ and $A_{int}$ is the specific interfacial area that can be computed from a correlation, e.g. \cite{SEM}. All other variables were previously defined and interphase mass transfer has been neglected. As for the single-phase Euler equations (\sct{sec:single-model}), the dissipative terms given in the vector $\mbold D\left( \mbold U_{liq} \right)$ were derived from the entropy minimum principle and have the following definitions:
%
\begin{multline}
...
%---------------------------------------------------------------------------------------------------
%
We now present two-phase flow results obtained with the seven-equation model described in \sct{sec:two-phase-model}. The EVM is used to stabilize the numerical solution in the presence of shock waves and other discontinuities. Results presented in this section include a two-phase flow shock tube (\sct{sec:2-phase-shock-tube}) and a water hammer like test (\sct{sec:two-num-res}). The objective of this section is to assess the performance of the EVM when simulating two-phase flows. Once again, all numerical presented in this section are run with linear polynomials as test function, the temporal integrator BDF2 and a second-order Gaussian quadrature rule using the RELAP-7 system code implementing the system of equations detailed in \sct{sct:model}.
%
%---------------------------------------------------------------------------------------------------
\subsubsection{Two-phase shock tubes} \label{sec:2-phase-shock-tube}
%---------------------------------------------------------------------------------------------------
%
We first investigate a two-phase flow shock tube in a 1-D pipe of length L = 36 m. The initial conditions consist of a left $(P, u, T, \alpha_{vap})_{left} = (15 \ MPa, 0. \ m/s, 615.3 \ K, 0.1)$ and a right $(P, u, T, \alpha_{vap})_{right} = (10 \ MPa, 0. \ m/s, 584.2 \ K, 0.5)$ chamber filled with a mixture of liquid water and vapor. The fluid properties are computed from the IAPWS95 Spline Based Table Lookup (see section 3.2.7 of \cite{relap7-manual}). The interfacial area $A_{int}$ used to compute the relaxation parameters $\mu_P$ and $\lambda_u$ (\eqt{eq:int_variables_def}) is set to 100. With such value, the relaxation terms force the phasic velocity and pressure to be equal (REF). Under this condition, the seven-equation model devolves to the five-equation model of Kapila (REF). At t = 0 s, the membrane located in the middle of the pipe is removed and the flow develop. The transient is run until t = 0.09 s with a CFL of 0.1 on a successive uniform mesh discretized with 100, 200, 400 and 800 cells. The same two-phase flow shock tube was previously run with RELAP-5 and WAHA system codes in \cite{Sokolowski-Koszela}. Numerical results obtained with the RELAP-7 system code are presented in \fig{fig:2p-shock-tube-plots}.
%
\begin{figure}[H]
        \vspace{-1 mm}
        \centering
        ...
        \begin{subfigure}[b]{0.49\textwidth}
                \centering
                \includegraphics[width=\textwidth]{figures/two-phase-shock-tube-pressure-plot.eps}
                \caption{Pressure}
                \label{fig:2p-shock-tube-plots-press}
        \end{subfigure}        
        \begin{subfigure}[b]{0.49\textwidth}
                \centering
                \includegraphics[width=\textwidth]{figures/two-phase-shock-tube-temperature-plot.eps}
                \caption{Temperature}
        ...
        \end{subfigure}                
        \caption{Numerical solutions of a two-phase flow shock tube at $t=9 \cdot 10^{-2} \ s$ for mesh with $100$, $200$, $400$ and $800$ cells.}\label{fig:2p-shock-tube-plots}
\end{figure}
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
Throughout this work we have presented two sets of results involving single- and two-phase flows developing pressure waves simulated with the RELAP-7 system code. The EVM was used to stabilize the numerical solutions with shock waves. Three shock tubes tests were simulated with air, liquid water and vapor and showed that the EVM can accurately resolved the shock, contact and rarefaction waves. Single-phase numerical results compared well against solutions obtained with the RELAP-5 and WAHA system codes from \cite{Sokolowski-Koszela}. Two-phase flow tests were also performed with the seven-equation model implemented in RELAP-7. The EVM was also used to stabilize the numerical solutions. Two cases were investigated, a shock tube and a water hammer-like test. TO FINISH.
