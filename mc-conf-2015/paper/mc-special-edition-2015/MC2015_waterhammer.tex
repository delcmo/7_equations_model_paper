...
\begin{array}{l}
 \mu_P \left( P_{gas} - P_{liq} \right) + \Gamma A_{int} / \rho_{int}\\
 \Gamma A_{int} \\
 \mu_P \left( P_{gas} - P_{liq} \right) + \Gamma A_{int} u_{int}\\
 \mu_P \bar{u}_{int} \left( P_{gas} - P_{liq} \right) + \lambda_u \bar{P}_{int} \left( u_{liq} - u_{gas} \right) - A_{int} h_{conv,liq} (T_{liq} - T_{int}) + \Gamma A_{int} E_{int}\\
\end{array}
\right], \,
\end{equation}
%
