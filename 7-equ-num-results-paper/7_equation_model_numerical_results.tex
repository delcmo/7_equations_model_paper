...
In this section, the regularized seven-equation two-phase flow model derived in (REF) is recalled along with the closure relations used to compute the relaxation terms in this paper. 
Then, the definitions of the phasic viscosity coefficients are derived from the low-Mach asymptotic study performed in (REF). 
%
%----------------------------------------------------------------------------------------------------------------------------------
...
so that we can quote the SEM independently of our previous work (the viscous regularization)}
%
%----------------------------------------------------------------------------------------------------------------------------------
\subsection{All-Mach definition of the phasic viscosity coefficients}\label{sec:visc-def}
%----------------------------------------------------------------------------------------------------------------------------------
%
\tcr{need short intro to this section to emphasize that from now on, this is NEW in the paper --that is the entropy viscosity coef for SEM}
...
\item INCLUDE two expressions for $A_{int}^{max}$: one for $\mu_P$ and one for $\lambda_u$. \tcr{why is this in this section? couldn't it be above?}
...
\end{equation}
%
where $h^K$ \tcr{I would drop the superscript $K$. You also omit it sometimes after, so maybe it's better not to put it in the first place}
is a characteristic size of the cell $K$ and $|| \cdot ||$ denotes the Euclidean norm. Note that an example of definition for $h^K$ when considering a cell K of volume $|K|$ belonging to a mesh of dimension $dim$ is $h^K = |K|^\frac{1}{dim}$. With the above definition of the first-order viscosity coefficients, the numerical solution was shown to be stable in REF for the case of a 1-D Sod shock test. The volume fraction equation (\eqt{eqn:multi-d-7-eqn-liq-vol}), however, does not require that much dissipation since its eigenvalue is the local interfacial velocity $\mbold u_{int}$ \tcr{we can talk about this}. Thus, we choose to set the viscosity coefficient $\beta_{k,max}$ used in the volume fraction proportional to the local interfacial velocity and keep the same definition for the viscosity coefficients $\mu_{k,max}$ and $\kappa_{k,max}$ as follows:
%
...
%
\tcr{I removed the $||$ around the jumps. By the way, the jumps are not defined}
...
\end{subequations}
%
\item Final definitions of the normalization parameters $\norm_{P,k}^\mu$ and $\norm_{P,k}^\kappa$ are determined by a low-Mach asymptotic limit of \eqt{eq:liq-7-eqn-sect5} in order to ensure well-scaled dissipative terms for all-Mach flows. The non-dimensional regularized SEM were derived in REF and are recalled in for the purpose of our study.
% 
...
used to solve the system of equations \eqt{eq:liq-7-eqn-sect5}. For conciseness, we re-write the system of 
