...
\tcr{An artificial viscosity technique for compressible multiphase flows}\\
}
%-------------------------
%-------------------------
\author{Marc O. Delchini\fnref{label1} \tcr{is that email correct for you?}}
...
(SEM) two-phase flow model described \cite{Berry_1985,BaerNunziato,Saurel_2001b,Berry_NED2010}.
...
we cannot just have the same text. Even if the reviewers are different, once published, we look like lazy fools!}
...
the viscous terms in Navier-Stokes equations \cite{references that JLG used in his JCP2011 paper} or by employing a parabolic regularization \cite{qqq}. 
...
minimum-entropy-satisfying viscous regularization \cite{guermond popov siam paper}; furthermore, Delchini et al. \cite{Marco_paper_low_mach, make sure the bibtex reference has been updated} 
...
\cite{jlg, jlg2, please change the name of these citations. I prefer to have longer names and have a clue to which paper we are referring to, such as GuermondJCP2011, GuermondCRA comptes rendus academiques, GuermondSIAM2014, ....}, 
...
single-phase flows \cite{jlg, jlg2}. 
...
%The theoretical foundation for this work can be traced back to viscous regularization for single-phase Euler and Navier-Stokes equations, notably \cite{jlg} and the references therein. 
%
...
by following the same methodology as in \cite{Marco_paper_low_mach}. In \sct{sec:disc}, the temporal and spatial discretization methods are presented. 
Our theoretical approach is illustrated in \sct{sec:results} with one-dimensional numerical results. The technique has been implemented in the nuclear 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
In this Section, we review the seven-equation two-phase flow model \cite{SEM} and recall the minimum-entropy-satisfying viscous 
...
start talking about the viscosities themselves....}
...
This is discussed next, in \sct{sec:visc-def}.
%  
...
%----------------------------------------------------------------------------------------------------------------------------------
%
\tcr{I wonder if this shouldn't be a new section to emphasize it is new, while the previous section would be everything that was already known....}
%
...
\sct{sec:results} to validate our approach.
...
and are denoted by the subscript $max$ and $e$, respectively. \tcr{that's just about notation. maybe a reader would like to know why there is an upper bound.
also, we never talked in terms of lower bound. where is this coming from???}
%
%-------------------------------------------------------------------------------------------------------------------
...
This approach has been successfully used to resolve shock waves for single-phase flows (see \cite{jlg1, jlg2, jlg3, why not our low mach paper and zingan?}. 

We first derive expressions for the high-order phasic viscosity coefficients $\mu_{e,k}$ and $\kappa_{e,k}$ and follow the same 
approach used in \cite{Marco_paper_low_mach} for the single-phase Euler equations. Such an approach is justified because the regularized SEM of \eqt{eq:7-eqn} 
degenerates to the regularized Euler equations studied in \cite{Marco_paper_low_mach} when one phase disappears.
...
In \cite{Marco_paper_low_mach}, we showed that recasting the entropy residual ($\resi(\mbold r,t)  := \matder{s}$) 
...
peculiarities related to the two-fluid system. We refer the reader to \cite{Marco_paper_low_mach} for the detailed derivations presented in the case of
single-phase flows.
\begin{enumerate}
%
...
\be
\mu_{k,e}    \propto \max\left( | \resi_k(\mbold r_q,t) |\,,  J_k  \right) \quad \text{and} \quad
...
dissipative terms for all-Mach flows. The non-dimensional regularized SEM were derived in \cite{REF} and are recalled in for the purpose of our study.
% 
...
\end{subequations}
%
where the superscript $*$ denotes a non-dimensional variable (\tcr{this is a bit short and you never explain what $\infty$ is either}). 
...
is obtained for $\Re_{k,\infty}$, $\Pe_{k,\infty}^\kappa$, and $\Pe_{k,\infty}^\beta$ in the two limit cases (a) and (b) given above \tcr{where???}, it will 
impose a condition for the definition of the phasic viscosity coefficients $\mu_k$, $\kappa_k$, and $\beta_k$. 
\end{enumerate} 
The non-dimensional regularized SEM \cite{Marco_paper_sem} were studied in two cases\tcr{too late. it is conceivable that this section will need more work. we need this fix 
the logic quickly ... time is running out before the semester ....}, the low-Mach asymptotic limit and non-isentropic flows, namely. The regularized 
...
\end{equation}
%
For non-isentropic flows i.e. with shocks, the scaling of the numerical numbers is given in \eqt{eq:non-isent-flow-num-nb} and is expected to efficiently 
...
Following the methodology employed in \cite{Marco_paper_low_mach} and the conditions given in \eqt{eq:non-isent-flow-num-nb}, the definition of the 
...
what is typically done for Burger's equation \cite{jlg1, jlg2}. Hence, we take $\beta_e$ to be proportional to the entropy residual, $R_{k}^\alpha$, derived from the 
...
\end{equation}
Using the new expression of the entropy residual $\resinew_k$, we now propose a definition, given in \eqt{eq:visc-coeff-ho-1} \tcr{why this eq number?}, for the phasic entropy 
...
and adding $(\alpha \rho A)_j,\, (\alpha \rho \mbold{u} A)_j,\, (\alpha \rho E A)_j$. you should do the same for F, N, R .... then the next section, 3.1.1, one understands that you tackled all 7 equations at the same and everything is clear...}
...
%Since there is no flux at the boundary coming from the integration by part of the hyperbolic  flux, the boundary value is imposed by using a Dirichlet boundary condition in the volume fraction equation.   
...
adapted for a time implicit solver \cite{Marco_dissertation}. The boundary type \tcr{do you mean the number of physical information to be supplied based on a study of the characteristics? if so, say it plainly}
is identified from the study of the sign of the eigenvalues that depends on the Mach number. The 
numerical results presented in 
\sct{sec:results} were all obtained by using subsonic stagnation and static pressure boundary conditions for the inlet and outlet, respectively\tcr{why is outlet always static?}. 
...
The first set \tcr{how many?} of 1-D numerical results is obtained using an application written using the multiphysics object-oriented simulation environment, MOOSE
\cite{moose}, and consists of various shock tube cases with both liquid and gas phases present. 
...
Next, a shock tube with two independent fluids is presented (\tcr{say how you make fluids independent}): 
such a setting is strictly identical to solving each phase independently with a single-phase code. 
Then, the same shock tube problem is considered but with infinite relaxation coefficients which degenerates the SEM to the five-equation model of Kapilla \cite{Kapila_2001}
(\tcr{say why you do this? to compare with Saurel 20xx results from \cite{abc}}). 
Finally, two-phase flow tests through a converging-diverging nozzle are considered. 
...
\tcr{need to say a couple of nice things about relap7, what does it do, etc. I am sure you can grab a couple of introduction sentence from a note that ray wrote on relap7....} 
...
For each test, data regarding the equations of state, the computational grid, and the time step used are provided. When an exact solution is available, a convergence study 
...
m^3$, respectively. \tcr{you use subscripts left/right and then 1/2. just use 1 and 2 all the time}
...
fraction variable only. The exact solution was obtained from a Riemann solver. Following \cite{}, the theoretical convergence rate for the L$_1$ and
...
\tcr{so, where do you test for the 2 options for beta max???? as stated in a remark in the theory section???}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This test was first introduced by Saurel et al. in \cite{SEM} and enables us to assess the behavior of the numerical method in the low-Mach asymptotic limit.
...
\tcb{Shoudl we add a few lines about R-7?} \tcr{earlier, see my other remark}
...
\tcb{Do we need to add plots of the transient solution?}  \tcr{do they look good????}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\tcr{not read yet but looks too short}
