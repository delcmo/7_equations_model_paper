...
Some properties of the Seven-Equation two-phase flow Model are discussed next. A set of $5+2\mathbb{D}$ (with $\mathbb{D}$ the geometry's dimension) waves is present in the model: two acoustic waves per phase, one contact wave per phase per domain dimension, and one void fraction wave propagating at the interfacial velocity $\mbold u_{int}$. These waves (eigenvalues of the Jacobian for the inviscid flux terms) and as follows for each phase $k$:
% 
...
\tcr{we should explain here or before the notations for clarity.}
...
\end{equation}
The terms $(s_e)_k$ and $(s_{\rho})_k$ denote the partial derivatives of the entropy $s_k$ with respect to $e_k$ and $\rho_k$, respectively.
...
\tcr{\\sign of delta P? should remain of same sign as $\div \mbold l_k$. mistake somewhere}\\
\tcr{i cannot get $\mbold u_{int}  \cdot \grad \rho_k$. the void equation has $\mbold u_{int} \cdot \grad \alpha_k$, how do you go from $\grad \alpha_k$ to $\grad \rho_k$?? }
%
...
\end{subequations}
%
However, the definition of $\mbold l_k$ proposed in \eqt{eq:def-l-k-wrong-sect4} was not considered as valid for the following reasons: positivity of the void fraction cannot be achieved \tcr{can you give ME some details as of why in my office one day?} and the parabolic regularization is not recovered when all viscosity coefficients are set equal.
\end{remark}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The first step consists of rearranging the equations given in \eqt{eq:sev_equ2-app} using the primitive variables $(\alpha_k, \rho_k, \mbold u_k, e_k)$, where $e_k$ is the specific internal energy of phase $k$. We introduce the material derivative $\frac{D (\cdot)}{Dt} = \partial_t (\cdot) + \mbold u_k \cdot \grad (\cdot)$ for simplicity. \\
