...
%-------------------------
\begin{abstract}
In this paper, a viscous regularization is derived for the Seven-Equation two-phase flow Model that ensures positivity of the entropy residual, uniqueness of the numerical solution when assuming concavity of the phasic entropy $s_k$, is consistent with the viscous regularization derived for Euler equations when one phase disappears and does not depend on the discretization scheme. It is also shown that the viscous regularization is compatible with the generalized Harten entropies.
\end{abstract}
%-------------------------
%-------------------------
\begin{keyword}
  two-phase flow model \sep with variable area \sep viscous regularization \sep artificial dissipative method \sep low Mach regime \sep shocks \sep continuous/discontinuous discretization schemes.
\end{keyword}
%-------------------------
\end{frontmatter}
\linenumbers
...
%\end{itemize}
%
Compressible two-phase flows are found in numerous industrial applications and are an ongoing area of research in modeling and simulation over many years. A variety of models with different levels of complexity has been developed such as: five-equation model of Kapila \cite{Kapila_2001}, six-equation model \cite{Toumi_1996}, and more recently the Seven-Equation two-phase flow Model \cite{SEM}. These models are all obtained by integrating the single-phase flow balance equations weighed by a characteristic or indicator function for each phase. The resulting system of equations contains non-conservative terms and relaxation terms that describe the interaction between phases but also an equation for the void fraction. Once a system of equations describing the physics is derived, the next challenging step is to develop a robust and accurate discretization to obtain a numerical solution. Two-phase flow system of equations are usually solved on a discontinuous discretization scheme. Thus, assuming that the system of equations is hyperbolic, a Riemann solver could be used but is often ruled out because of the complexity due to the number of equations involved. An alternative is to use an approximate Riemann solver, a well-established approach for single-phase flows, while ensuring the correct low-Mach asymptotic limit and deriving a consistent discretization scheme for the non-conservative terms \cite{Saurel_2001a, Saurel_2001b, Li_2004, Zein_2010, Ambroso_2012, Abgrall_2002}. 
...
In this paper, it is proposed to derive a viscous regularization for the Seven-Equation two-phase flow Model that is consistent with the entropy minimum principle on the same model as what is done for Euler equations with/without variable area \cite{jlg, Marco_paper_low_mach}. Unlike approximate Riemann solvers, artificial dissipative method can be used with both continuous and discontinuous discretization schemes. 

First, the Seven-equation two-phase flow Model is recalled along with its main properties. Then, the reader is guided through the steps leading to the derivation of the viscous regularization. Lastly, the scaled Seven-Equation two-phase flow Model with viscous regularization are derived to give insights on what the scaling of the dissipative terms should be to ensure well-scaled dissipative terms for a wide range of Mach flows.
%
...
The set of equations obeyed by phase $j$ are simply obtained by substituting $k$ by $j$ and $j$ by $k$ in \eqt{eq:liq-7-eqn-sect5}, keeping the same definition of the interfacial variables and noting that $\Gamma_j = - \Gamma_k$. In the case of two-phase flows, the equation for the void fraction of phase $j$ is simply replaced by the algebraic relation
%
\begin{align}
 \alpha_{j}= 1 - \alpha_{k}, \nonumber
\end{align}
...
Properties of the Seven-Equation two-phase flow Model are discussed next. A set of seven waves is present in such a model: two acoustic waves, a contact wave for each phase and by a void fraction wave propagating at the interfacial velocity $\mbold u_{int}$. Considering a spatial domain of dimension $\mathbb{D}$, the corresponding eigenvalues are the following for each phase $k$:
% 
...
Seven-Equation two-phase flow Model not to the classical six-equation model but to the
...
\end{equation}
%
Note that when one phase disappears, \eqt{eq:tot-ent-res-sct4} degenerates into the single phase entropy equation obtained from the multi-dimensional Euler equations \cite{SEM, Marco_dissertation}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}\label{eq:conclusion}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Deriving a definition for the phasic viscosity coefficients should rely on existing numerical methods for scalar and system of hyperbolic equations. For instance, it is known that artificial dissipative methods are used to solved for Euler equations: Lapidus \cite{Lapidus_paper, Lapidus_book}, pressure-based \cite{PBV_book} and entropy-based \cite{jlg1, valentin} numerical methods. Once a definition for the viscosity coefficients is derived and found consistent with the scaling of the numerical non-dimensionalized numbers, the numerical methods can be tested by solving two-phase shock tubes using various discretization methods. Note that the viscous regularization proposed in this paper is discretization independent. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
